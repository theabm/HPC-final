...
\begin{lstlisting}[language=C++]
//copy buffer into buffer of new generation
memcpy(data+cols, data_prev+cols, grid_size_bytes);

// copy last row into top halo
...
// copy top halo back into last row
memcpy(data + rows_x_cols, data, row_len_bytes);

// copy row 1 into bottom halo 
memcpy(data+rows_x_cols_p_cols, data+cols, row_len_bytes);

// we go through the rest of the array (from row 3 to last row)
...
// we copy bottom halo into row 1
...
void upgrade_cell_static(unsigned char * restrict data_prev, unsigned char * restrict data, unsigned long int i, unsigned long int j)
{
    ...
    // of data_prev, we only need to keep track of 
    ...
    { 
        ...
        if(count_of_neighbors == 3) 
        {
            ...
            // of each one. // To do this, we need to sum 0x02
