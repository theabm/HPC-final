// THIS IS VERSION V2 WHICH USES THE PBM FILE FORMAT. IN THIS CASE 
// WE ARE DOING EVERYTHING AT BIT LEVEL WHICH MEANS THAT WE ARE USING 
// 8 TIMES LESS MEMORY COMPARED TO V1, HOWEVER THE COMPLEXITY IS A BIT 
// HIGHER.
// SOME KEY DIFFERENCES ARE:
// THE FILE HEADER MAGIC NUMBER IS P4 AND IT DOES NOT REQUIRE A MAXVAL PARAMETER
// THE EXTENSION IS .PBM
// DEAD OR ALIVE IS ENCODED AS 0 OR 1 
//
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
...
int main(int argc, char **argv){
    ...
    if(action == INIT){
        ...
        int header_size = snprintf(NULL, 0, "P4 %d %d ", rows, cols);
        char * header = malloc(header_size + 1);
        sprintf(header, "P4 %d %d ", rows, cols);
